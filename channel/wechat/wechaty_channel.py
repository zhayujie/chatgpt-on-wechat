...
class WechatyChannel(Channel):
    ...
    async def on_scan(
        ...
        logger.info(
            "[WX] scan user={}, scan status={}, scan qr_code={}".format(
    ...
    async def on_message(self, msg: Message):
        ...
        if room is None and msg.type() == MessageType.MESSAGE_TYPE_TEXT:
            ...
            config = conf()
            match_prefix = (
                (is_at and not config.get("group_at_off", False))
            ...
            if (
                ...
                or self.check_contain(
                    room_name, config.get("group_name_keyword_white_list")
                )
            ) and match_prefix:
                img_match_prefix = self.check_prefix(
                    content, conf().get("image_create_prefix")
                ...
                if img_match_prefix:
                    ...
                    await self._do_send_group(
                        content, room_id, room_name, from_user_id, from_user_name
                    )

    async def send(
        self,
        message: Union[str, Message, FileBox, Contact, UrlLink, MiniProgram],
        receiver,
    ):
    ...
    async def _do_send_group(
        self, query, group_id, group_name, group_user_id, group_user_name
    ):
        if not query:
            return
        context = dict()
        ...
        if reply_text:
            ...
            await self.send_group(
                conf().get("group_chat_reply_prefix", "") + reply_text, group_id
            )
