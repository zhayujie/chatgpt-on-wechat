...
class WechatMessage(ChatMessage):
    def __init__(self, itchat_msg, is_group=False):
        ...
        if itchat_msg["Type"] == TEXT:
            ...
            #todo add keyword checks for English version wechat 
            ...
            elif "你已添加了" in itchat_msg["Content"]:  #通过好友请求 #todo added bug, when contain added string then it will go accept_friend type if tickle the bot
                self.ctype = ContextType.ACCEPT_FRIEND
                self.content = itchat_msg["Content"]
            elif "拍了拍我" or "tickled" in itchat_msg["Content"]:
                self.ctype = ContextType.PATPAT
                self.content = itchat_msg["Content"]
                #done get the expressions before the "tickled"
                if is_group:
                    try:
                        self.actual_user_nickname = re.findall(r"\"(.*?)\"", itchat_msg["Content"])[0]
                    except Exception:
                        self.actual_user_nickname = re.findall(r"([\w]+)(?= tickled)", itchat_msg["Content"])[0] 
            else:
